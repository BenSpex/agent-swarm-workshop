...
### 16. TODO-INTEGRATION Markers Are NOT a Soft Gate (Runs 10, 11, 13 all hit this)
Three runs in a row, same pattern: Core team committed `// TODO-INTEGRATION` identity-function stubs in `actionHandlers.ts` + `tickHandler.ts` (no-op calls for `getProjectById` and the 8 subsystem updaters) even though Systems had already shipped the `../systems` barrel. The "we'll replace them later" intent never materialized — orchestrator patched post-merge each time. Root cause: Core team-lead Claude builds in parallel, doesn't `git fetch origin systems` before committing, and the prompt's TODO-INTEGRATION "fallback" reads as permission rather than a last resort. **Run 14 fix:** (a) `scripts/verify-ownership.sh` rejects any branch containing `TODO-INTEGRATION` in `src/` (Check 5, already in script); (b) team-core.md now has a hard pre-commit grep that blocks commits with any `TODO-INTEGRATION` marker in `src/core/`; (c) team-core.md now requires `git fetch origin systems && git merge origin/systems` before every commit. Orchestrator must run `bash scripts/verify-ownership.sh <branch>` BEFORE every merge — if it exits 1, route back to team via `.swarm/tasks-{team}.md` and do NOT merge.
